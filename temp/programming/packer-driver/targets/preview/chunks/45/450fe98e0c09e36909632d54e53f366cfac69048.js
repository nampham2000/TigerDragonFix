      ...
      _export("NetworkConnect", NetworkConnect = (_dec = ccclass("NetworkConnect"), _dec2 = property({
        ...
        connect() {
          ...
          return _asyncToGenerator(function* () {
            try {
              ...
              if (rooms.length === 0) {
                _this2.room = yield _this2.client.create("room1");
                console.log("Created new room with sessionId:", _this2.room.sessionId);
              } else {
                // Nếu có phòng có sẵn, tham gia vào phòng đầu tiên trong danh sách
                _this2.room = yield _this2.client.joinById(rooms[0].roomId);
                console.log("Joined existing room with sessionId:", _this2.room.sessionId);
              }

              var users = {}; // Sự kiện được kích hoạt khi một người dùng tham gia vào phòng

              console.log("Joined successfully!");
              console.log("User's sessionId:", _this2.room.sessionId);
              ...
              _this2.room.onMessage("userCancel", message => {
                var usercancel = _this2.room.state.players.get(message.playerId).displayName;

                for (var i = 0; i < _this2.ListLabel.length; i++) {
                  if (usercancel === _this2.ListLabel[i].string) {
                    _this2.ListLabel[i].node.removeAllChildren();
                  }
                }

                for (var _i = 0; _i < _this2.ListLabelR.length; _i++) {
                  if (usercancel === _this2.ListLabelR[_i].string) {
                    _this2.ListLabelR[_i].node.removeAllChildren();
                  }
                }
              ...
              _this2.room.onMessage("userBet", message => {
                ...
                _this2.namePlayer = _this2.room.state.players.get(message.playerID);

                if (message.playerID != _this2.room.sessionId) {
                  _this2.UserBet = message.betType;

                  _this2.createSpriteNode(_this2.namePlayer.displayName, message.betAmount);
        ...
        updatePlayerList(playerList) {
          ...
          list.forEach((value, key) => {
            ...
            var previousHostIndex = this.ListLabelR.findIndex(label => label.string === this.currentHost);

            if (previousHostIndex !== -1 && value.sessionId !== this.currentHost) {
              // Đưa phần tử trước đó là currentHost xuống cuối mảng
              var previousHost = this.ListLabelR.splice(previousHostIndex, 1)[0];
              this.ListLabelR.push(previousHost); // Cập nhật trạng thái của phần tử này

              this.ListR[previousHostIndex].active = false;
              this.ListR.push(this.ListR.splice(previousHostIndex, 1)[0]); // Điều chỉnh displayIndexR nếu cần thiết
          ...
          for (var i = displayIndex; i < this.ListL.length; i++) {
            this.ListL[i].active = false;
          }

          for (var _i2 = displayIndexR; _i2 < this.ListR.length; _i2++) {
            this.ListR[_i2].active = false;
          }
        }
        ...
        createSpriteNode(sessionId, betAmount) {
          ...
          var prefab = this.prefabs[betAmount - 1];
          var spriteNode = instantiate(prefab);
          ...
          for (var _i3 = 0; _i3 < this.ListLabelR.length; _i3++) {
            if (sessionId === this.ListLabelR[_i3].string) {
              // Thêm node mới làm con của node có label tương ứng
              if (this.UserBet === "Dragon") {
                PosTarget = this.DragonNode;
                this.parentNodeChip = this.ChipParent[0];
              ...
              this.ListLabelR[_i3].node.addChild(spriteNode);

              this.ListLabelR[_i3].node.inverseTransformPoint(v3, PosTarget.worldPosition);
        ...
        PayoutAnim(winnerList) {
          ...
          for (var i = 0; i < winnerList.length; i++) {
            var winner = this.room.state.players.get(winnerList[i].sessionId).displayName; // Tìm node tương ứng với người chiến thắng
...
//# sourceMappingURL=450fe98e0c09e36909632d54e53f366cfac69048.js.map